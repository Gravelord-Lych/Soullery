...
import lych.soullery.block.entity.EnchanterSpawnerTileEntity;
...
public class SkyCityPieces {
    ...
    private static final Selection<BlockState> SLABS_SELECTION = Selections.selection(makeItem(ModBlocks.PROFOUND_STONE_BRICKS.slab().defaultBlockState(), 4), makeItem(ModBlocks.CRACKED_PROFOUND_STONE_BRICKS.slab().defaultBlockState(), 1));
    private static final StructurePiece.BlockSelector SLABS = WorldUtils.selectorFrom(SLABS_SELECTION);
    ...
    private static final Selection<BlockState> WALLS = Selections.selection(makeItem(ModBlocks.PROFOUND_STONE_BRICKS.wall().defaultBlockState(), 4), makeItem(ModBlocks.CRACKED_PROFOUND_STONE_BRICKS.wall().defaultBlockState(), 1));
    public static final BlockState FENCE = Blocks.WARPED_FENCE.defaultBlockState();
    ...
    public static class BossRoom extends BasePiece {
        private static final Vector3i[] DISASSEMBLER_OFFSETS = new Vector3i[]{new Vector3i(12, 0, 12), new Vector3i(12, 0, -12), new Vector3i(-12, 0, 12), new Vector3i(-12, 0, -12)};
        ...
        public boolean postProcess(ISeedReader reader, StructureManager manager, ChunkGenerator generator, Random random, MutableBoundingBox bb, ChunkPos chunkPos, BlockPos blockPos) {
            ...
            for (int x = ct - 1; x <= ct + 1; x++) {
                for (int y = 5; y < MAX_Y - 4; y++) {
                    gate.add(new BlockPos(getWorldX(x, 0), getWorldY(y), getWorldZ(x, 0)));
                }
            }
            ...
            int altarY = 5;
            generateBox(reader, bb, ct - 1, altarY, ct - 1, ct + 1, altarY, ct + 1, false, random, SLABS);
            placeBlock(reader, BRICKS_SELECTION.getRandom(random), ct, altarY, ct, bb);
            placeBlock(reader, Blocks.CHAIN.defaultBlockState(), ct, altarY + INNER_BOSS_ROOM_HEIGHT - 1, ct, bb);
            placeBlock(reader, Blocks.SOUL_LANTERN.defaultBlockState().setValue(LanternBlock.HANGING, true), ct, altarY + INNER_BOSS_ROOM_HEIGHT - 2, ct, bb);

            List<BlockPos> disassemblers = new ArrayList<>();
            for (Vector3i vec : DISASSEMBLER_OFFSETS) {
                int x = ct + vec.getX();
                int y = altarY + 1;
                int z = ct + vec.getZ();
                placeBlock(reader, ModBlocks.DISASSEMBLER.defaultBlockState(), x, y, z, bb);
                placeBlock(reader, WALLS.getRandom(random), x, y - 1, z, bb);
                disassemblers.add(new BlockPos(getWorldX(x, z), getWorldY(y), getWorldZ(x, z)));
            }

            BlockPos ac1 = new BlockPos(getWorldX(ct - INNER_BOSS_ROOM_SIZE / 2, ct - INNER_BOSS_ROOM_SIZE / 2), getWorldY(altarY), getWorldZ(ct - INNER_BOSS_ROOM_SIZE / 2, ct - INNER_BOSS_ROOM_SIZE / 2));
            BlockPos ac2 = new BlockPos(getWorldX(ct + INNER_BOSS_ROOM_SIZE / 2, ct + INNER_BOSS_ROOM_SIZE / 2), getWorldY(altarY + INNER_BOSS_ROOM_HEIGHT - 1), getWorldZ(ct + INNER_BOSS_ROOM_SIZE / 2, ct + INNER_BOSS_ROOM_SIZE / 2));
            Arena arena = Arena.link(ac1, ac2);
            if (!placedBossSpawner) {
                placedBossSpawner = WorldUtils.placeBlockEntity(this::placeBlock, this::getWorldX, this::getWorldY, this::getWorldZ, reader, ModBlocks.ENCHANTER_SPAWNER.defaultBlockState(), EnchanterSpawnerTileEntity.class, ct, altarY + 2, ct, bb, spawner -> {
                    spawner.setArena(arena);
                    spawner.setRange(INNER_BOSS_ROOM_SIZE / 2);
                    spawner.setVerticalRange(2);
                    gate.forEach(spawner::addGateBlock);
                    disassemblers.forEach(spawner::addDisassembler);
                });
            }
    ...
    public static class SoulStalkRoom extends BasePiece {
        ...
        @Nullable
        public static SkyCityPieces.SoulStalkRoom createPiece(List<StructurePiece> pieces, int x0, int y0, int z0, Direction direction, int genDepth) {
            MutableBoundingBox boundingBox = MutableBoundingBox.orientBox(x0, y0, z0, -5, -3, 0, 13, 14, 13, direction);
